          ...
          {appPhase === 'idle' && (
            ...
            >
              ...
              <div className="flex gap-2 mb-4">
                <button
                  onClick={runAnalysis}
                  style={{ backgroundColor: "#0A0A0A", color: "#fff" }} className="flex-1 py-2.5 rounded-lg text-[13px] font-semibold hover:opacity-90 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                >
                  <Satellite size={15} />
                  CNN 분석 실행
                </button>
              ...
              <div className="grid grid-cols-6 gap-2 mb-5">
                {YEARS.map(yr => (
                  <div key={yr} className="aspect-square rounded-lg overflow-hidden border border-border bg-neutral-100 relative group/thumb hover:border-border2 transition-colors">
                    ...
                    <div className="absolute bottom-1 left-1/2 -translate-x-1/2 group-hover/thumb:opacity-0 transition-opacity">
                ...
                <div className="border border-border rounded-card bg-white p-5">
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <AreaChart data={NDVI_DATA} margin={{ top: 10, right: 20, bottom: 5, left: 0 }}>
                      ...
                      <ReferenceLine y={0.6} stroke="#a1a1aa" strokeDasharray="6 4" strokeWidth={1.5} />
                      <ReferenceLine x="2020" stroke="#d4d4d8" strokeDasharray="4 4" strokeWidth={1.5} />
                      <Area type="monotone" dataKey="ndvi" stroke="#6366f1" strokeWidth={2.5} fill="url(#ndviGrad)" dot={{ r: 5, fill: '#6366f1', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 7, fill: '#4f46e5' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
